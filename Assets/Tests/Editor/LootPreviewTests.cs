...
namespace NS.UnifiedLoot.Tests {
    public class LootPreviewTests {
        ...
        public void LootPreview_CorrectlyReflectsWeightAndQuantityModifications() {
            ...
            Assert.AreEqual((double)14f, preview.TotalWeight, 0.001);
            Assert.AreEqual(2, preview.Entries.Count);

            var swordEntry = preview.Entries[0];
            Assert.AreEqual(TestItem.Sword, swordEntry.Item);
            Assert.AreEqual((double)5f, swordEntry.OriginalWeight, 0.001);
            Assert.AreEqual((double)10f, swordEntry.ModifiedWeight, 0.001);
            ...
            Assert.AreEqual((double)2f, potionEntry.OriginalWeight, 0.001);
            Assert.AreEqual((double)4f, potionEntry.ModifiedWeight, 0.001);
